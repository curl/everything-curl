...
All headers set by curl itself can be replaced, by the user. You then tell
curl's `-H` or `--header` the new header to use and it then replaces the
...
If you want to delete an internally generated header, give it to curl without
a value, nothing on the right side of the colon.
