...
curl gets exactly what you ask it to get and it never parses the actual
...
files. When curl downloads HTML it gets that single HTML resource, even if it,
when parsed by a browser, would trigger a whole busload of more downloads. If
you want curl to download any sub-resources as well, you need to pass those
URLs to curl and ask it to get those, like any other URLs.
...
compressed data, or telling the server from which previous page we are "coming
from". curl's requests differ a little or a lot from how your browser sends
its requests.
