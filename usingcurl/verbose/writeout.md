...
| `tls_earlydata`           | The amount of bytes that were sent as TLS 1.3 early data. This is zero if this TLS feature was not used and negative if the data sent was rejected by the server. The use of early data is enabled with `--tls-earlydata`.(Introduced in 8.12.0) |
